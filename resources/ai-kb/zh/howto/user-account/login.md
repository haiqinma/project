...
Project 只配置 `IDENTITY_TRUST_DIR`（生产默认目录为 `/data/node`）。独立定时任务从 Node 同步并校验 `issuer-metadata.json`、`jwks.json` 和 `manifest.json`，Project 登录时只读取该目录并校验文件摘要，不实时访问 Node。通行证登录另行使用 `IDENTITY_NODE_URL` 和应用授权配置。
