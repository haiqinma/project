...
Local working files can be kept outside the release independently by setting `YEYING_RUNTIME_DIR` in `.env`. It defaults to `.`, which preserves local-development behavior. Production should use a stable absolute directory, such as `/opt/deploy/shared/project/runtime`; `scripts/install.sh` moves and links temporary uploads, framework runtime directories, logs and PID files there. This does not move persistent uploads: local mode still requires shared `public/uploads`, and S3 mode still uses the object store as the source of truth.
...
For production operations, use the step-by-step runbook in `docs/S3存储迁移操作手册.md`.
